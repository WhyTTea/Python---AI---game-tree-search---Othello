...
def alphabeta_min_node(board, color, alpha, beta, limit, caching=0, ordering=0):
    ...
    if len(successor_moves) == 0 or (limit == 0):
        ...
        if ordering == 1:    
            node_ordering = []
            for move in successor_moves:
                new_board_state = play_move(board, opp_color, move[0], move[1])
                node_ordering.append((move, compute_utility(new_board_state, color), new_board_state))
            node_ordering.sort(key=lambda x: x[1])
            # successor_moves = [item[0] for item in node_ordering]
            min_move = None
            min_util = float("inf")
            for move, util, board_state in node_ordering:
                # next_board_state = play_move(board, opp_color, move[0], move[1])
                next_move, next_util = alphabeta_max_node(board_state, color, alpha, beta, limit - 1, caching)
                if min_util > next_util:
                    if next_util < beta:
                        beta = next_util
                    min_util = next_util
...
def alphabeta_max_node(board, color, alpha, beta, limit, caching=0, ordering=0):
    ...
    if (len(successor_moves) == 0) or (limit == 0):
        ...
        if ordering == 1:
            ...
            for move, util, board_state in node_ordering:
                # next_board_state = play_move(board, color, move[0], move[1])
                next_move, next_util = alphabeta_min_node(board_state, color, alpha, beta, limit - 1, caching)
                if max_util < next_util:
                    if alpha < next_util:
                        alpha = next_util
                    max_util = next_util
